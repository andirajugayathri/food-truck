document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.custom-select-wrapper').forEach(wrapper => {
        ...
        options.forEach(option => {
            option.addEventListener('click', function (e) {
                // If clicking the label/div, toggle the checkbox
        ...
        function updateTriggerText() {
            ...
            inputs.forEach(input => {
                if (input.checked) {
                    // Get the text content of the parent label, trimming whitespace
                    selectedValues.push(input.parentElement.textContent.trim());
                }
            });

            if (selectedValues.length === 0) {
                triggerText.textContent = 'Coverage Required';
            } else if (selectedValues.length <= 2) {
                triggerText.textContent = selectedValues.join(', ');
            } else {
                triggerText.textContent = `${selectedValues.length} Selected`;
            }
        }
    ...
    // FAQ Accordion Logic
    const faqQuestions = document.querySelectorAll('.faq-question, .idx2-faq-question');

    faqQuestions.forEach(question => {
        question.addEventListener('click', () => {
            const item = question.closest('.faq-item, .idx2-faq-item');
            if (!item) return;
            
            const isActive = item.classList.contains('active');

            // Close all other items
            document.querySelectorAll('.faq-item, .idx2-faq-item').forEach(otherItem => {
                otherItem.classList.remove('active');
                const otherAnswer = otherItem.querySelector('.faq-answer, .idx2-faq-answer');
            ...
            if (!isActive) {
                ...
                const answer = item.querySelector('.faq-answer, .idx2-faq-answer');
                if (answer) {
